...
// RecordUpsert is a [recordmodels.Record] upsert (create/update) form.
...
// config created from the provided [core.App] and [recordmodels.Record] instances
// (for create you could pass a pointer to an empty Record - models.NewRecord(collection)).
//
// If you want to submit the form as part of a transaction,
// you can change the default Dao via [SetDao()].
func NewRecordUpsert(app core.App, record *recordmodels.Record) *RecordUpsert {
	form := &RecordUpsert{
		app:           app,
		dao:           app.Dao(),
		record:        record,
...
// (often used with users collection records).
...
func (form *RecordUpsert) loadFormDefaults() {
	...
	if form.record.Collection().IsUsers() {
...
func (form *RecordUpsert) extractMultipartFormData(
	...
	data := map[string]any{}
	filesToUpload := map[string][]*filesystem.File{}
	arraybleFieldTypes := collectionmodels.ArraybleFieldTypes()
...
func (form *RecordUpsert) AddFiles(key string, files ...*filesystem.File) error {
	field := form.record.Collection().Schema.GetFieldByName(key)
	if field == nil || field.Type != collectionmodels.FieldTypeFile {
		return errors.New("invalid field key")
	}

	options, ok := field.Options.(*collectionmodels.FileOptions)
	if !ok {
...
func (form *RecordUpsert) LoadData(requestData map[string]any) error {
	...
	if v, ok := requestData[collectionmodels.FieldNameId]; ok {
		form.Id = cast.ToString(v)
	}

	// load auth system fields
	if form.record.Collection().IsUsers() {
		if v, ok := requestData[collectionmodels.FieldNameUsername]; ok {
			form.Username = cast.ToString(v)
		}
		if v, ok := requestData[collectionmodels.FieldNameEmail]; ok {
			form.Email = cast.ToString(v)
		}
		if v, ok := requestData[collectionmodels.FieldNameEmailVisibility]; ok {
			form.EmailVisibility = cast.ToBool(v)
		}
		if v, ok := requestData[collectionmodels.FieldNameVerified]; ok {
...
func (form *RecordUpsert) Validate() error {
	...
	baseFieldsRules := []*validation.FieldRules{
		validation.Field(
			...
			validation.When(
				form.record.IsNew(),
				validation.Length(recordmodels.DefaultIdLength, recordmodels.DefaultIdLength),
	...
	if form.record.Collection().IsUsers() {
		baseFieldsRules = append(baseFieldsRules,
			...
			validation.Field(
				&form.Email,
				validation.When(
					form.record.Collection().UserOptions().RequireEmail,
					validation.Required,
				),
			...
			validation.Field(
				...
				validation.Length(form.record.Collection().UserOptions().MinPasswordLength, 72),
			),
			validation.Field(
...
func (form *RecordUpsert) checkUniqueEmail(value any) error {
	...
	isUnique := form.dao.IsRecordValueUnique(
		form.record.Collection().Id,
		collectionmodels.FieldNameEmail,
		v,
		form.record.Id,
	)
	if !isUnique {
...
func (form *RecordUpsert) checkEmailDomain(value any) error {
	...
	only := form.record.Collection().UserOptions().OnlyEmailDomains
	except := form.record.Collection().UserOptions().ExceptEmailDomains
...
func (form *RecordUpsert) ValidateAndFill() error {
	...
	if form.record.Collection().IsUsers() {
		...
		if form.record.IsNew() && form.Username == "" {
			...
			form.Username = form.dao.SuggestUniqueUserRecordUsername(form.record.Collection().Id, baseUsername)
		}
...
func (form *RecordUpsert) Submit(interceptors ...baseforms.InterceptorFunc[*recordmodels.Record]) error {
	if err := form.ValidateAndFill(); err != nil {
		return err
	}

	return baseforms.RunInterceptors(form.record, func(record *recordmodels.Record) error {
		form.record = record
		...
		// ---
		dao.BeforeCreateFunc = func(eventDao *daos.Dao, m recordmodels.Model, action func() error) error {
		...
		dao.BeforeUpdateFunc = func(eventDao *daos.Dao, m recordmodels.Model, action func() error) error {
			newAction := func() error {
				if m.TableName() == form.record.TableName() && m.GetId() == form.record.GetId() {
					var err error
					uploaded, err = form.processFilesToUpload()
