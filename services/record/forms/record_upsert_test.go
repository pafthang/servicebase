...
func hasRecordFile(app core.App, record *recordmodels.Record, filename string) bool {
...
func TestNewRecordUpsert(t *testing.T) {
	...
	form := recordforms.NewRecordUpsert(app, record)
...
func TestRecordUpsertDrySubmitFailure(t *testing.T) {
	...
	form := recordforms.NewRecordUpsert(app, recordBefore)
	req := httptest.NewRequest(http.MethodGet, "/", formData)
	req.Header.Set(echo.HeaderContentType, mp.FormDataContentType())
...
func TestRecordUpsertSubmitFailureFilesCleanup(t *testing.T) {
	...
	for _, s := range scenarios {
		...
		t.Run(sname, func(t *testing.T) {
			...
			var record *recordmodels.Record
			if s.recordId == "" {
				record = recordmodels.NewRecord(collection)
			} else {
...
func TestRecordUpsertWithCustomId(t *testing.T) {
	...
	scenarios := []struct {
	...
	}{
		{
			"empty data",
			map[string]string{},
			recordmodels.NewRecord(collection),
			false,
		},
		{
			"empty id",
	...
	for _, scenario := range scenarios {
		...
		form := recordforms.NewRecordUpsert(app, scenario.record)
		req := httptest.NewRequest(http.MethodGet, "/", formData)
		req.Header.Set(echo.HeaderContentType, mp.FormDataContentType())
		form.LoadRequest(req, "")
...
func TestRecordUpsertAuthRecord(t *testing.T) {
	...
	for _, s := range scenarios {
		...
		if s.existingId != "" {
			...
			if err != nil {
				t.Errorf("[%s] Failed to fetch users record with id %s", s.name, s.existingId)
				continue
			}
		}
...
func TestRecordUpsertUniqueValidator(t *testing.T) {
	...
	dummyRecord := recordmodels.NewRecord(collection)
...
func TestRecordUpsertUploadFailure(t *testing.T) {
	...
	for i, dao := range testDaos {
		// create with invalid file
		{
			...
			new := recordmodels.NewRecord(collection)
			new.Id = "123456789012341"

			form := recordforms.NewRecordUpsert(app, new)
			form.SetDao(dao)
