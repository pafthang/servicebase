...
import (
	...
	"github.com/pafthang/servicebase/tools/store"
...
type Record struct {
	...
	ignoreEmailVisibility bool // whether to ignore the emailVisibility flag for users collections
	loaded                bool
...
func NewRecord(collection *collectionmodels.Collection) *Record {
	return &Record{
...
func NewRecordFromNullStringMap(collection *collectionmodels.Collection, data dbx.NullStringMap) *Record {
	...
	for _, name := range collectionmodels.BaseModelFieldNames() {
		resultMap[name] = nullStringMapValue(data, name)
	}

	// load users collection fields
	if collection.IsUsers() {
		for _, name := range collectionmodels.UserFieldNames() {
			resultMap[name] = nullStringMapValue(data, name)
		}
	}
...
func NewRecordsFromNullStringMaps(collection *collectionmodels.Collection, rows []dbx.NullStringMap) []*Record {
...
func (m *Record) Collection() *collectionmodels.Collection {
	return m.collection
}
...
// IgnoreEmailVisibility toggles the flag to ignore the users record email visibility check.
...
func (m *Record) Set(key string, value any) {
	switch key {
	case collectionmodels.FieldNameId:
		m.Id = cast.ToString(value)
	case collectionmodels.FieldNameCreated:
		m.Created, _ = types.ParseDateTime(value)
	case collectionmodels.FieldNameUpdated:
		m.Updated, _ = types.ParseDateTime(value)
	case collectionmodels.FieldNameExpand:
	...
	default:
		...
		} else if m.collection.IsUsers() {
			// normalize users collection fields
			switch key {
			case collectionmodels.FieldNameEmailVisibility, collectionmodels.FieldNameVerified:
				v = cast.ToBool(value)
			case collectionmodels.FieldNameLastResetSentAt, collectionmodels.FieldNameLastVerificationSentAt, collectionmodels.FieldNameLastLoginAlertSentAt:
				v, _ = types.ParseDateTime(value)
			case collectionmodels.FieldNameUsername, collectionmodels.FieldNameEmail, collectionmodels.FieldNameTokenKey, collectionmodels.FieldNamePasswordHash:
...
func (m *Record) FindFileFieldByFile(filename string) *collectionmodels.SchemaField {
	for _, field := range m.Collection().Schema.Fields() {
		if field.Type == collectionmodels.FieldTypeFile {
...
func (m *Record) ColumnValueMap() map[string]any {
	...
	// export users collection fields
	if m.collection.IsUsers() {
		for _, name := range collectionmodels.UserFieldNames() {
	...
	result[collectionmodels.FieldNameId] = m.getNormalizeDataValueForDB(collectionmodels.FieldNameId)
	result[collectionmodels.FieldNameCreated] = m.getNormalizeDataValueForDB(collectionmodels.FieldNameCreated)
	result[collectionmodels.FieldNameUpdated] = m.getNormalizeDataValueForDB(collectionmodels.FieldNameUpdated)
...
//
// For users records, to force the export of the email field you need to set
...
func (m *Record) PublicExport() map[string]any {
	...
	// export some of the safe users collection fields
	if m.collection.IsUsers() {
		result[collectionmodels.FieldNameVerified] = m.Verified()
		result[collectionmodels.FieldNameUsername] = m.Username()
		result[collectionmodels.FieldNameEmailVisibility] = m.EmailVisibility()
		if m.ignoreEmailVisibility || m.EmailVisibility() {
			result[collectionmodels.FieldNameEmail] = m.Email()
		}
	}

	// export base model fields
	result[collectionmodels.FieldNameId] = m.GetId()
	if created := m.GetCreated(); !m.Collection().IsView() || !created.IsZero() {
		result[collectionmodels.FieldNameCreated] = created
	}
	if updated := m.GetUpdated(); !m.Collection().IsView() || !updated.IsZero() {
		result[collectionmodels.FieldNameUpdated] = updated
	}

	// add helper collection reference fields
	result[collectionmodels.FieldNameCollectionId] = m.collection.Id
	result[collectionmodels.FieldNameCollectionName] = m.collection.Name

	// add expand (if set)
	if m.expand != nil && m.expand.Length() > 0 {
		result[collectionmodels.FieldNameExpand] = m.expand.GetAll()
	}
...
func (m *Record) ReplaceModifers(data map[string]any) map[string]any {
	...
	modifiers := collectionmodels.FieldValueModifiers()
...
func (m *Record) extractUnknownData(data map[string]any) map[string]any {
	...
	for _, name := range collectionmodels.SystemFieldNames() {
...
var notUserRecordErr = errors.New("Not a users collection record.")

// Username returns the "username" users record data value.
func (m *Record) Username() string {
	return m.GetString(collectionmodels.FieldNameUsername)
}

// SetUsername sets the "username" users record data value.
//
// This method doesn't check whether the provided value is a valid username.
//
// Returns an error if the record is not from a users collection.
func (m *Record) SetUsername(username string) error {
	if !m.collection.IsUsers() {
		return notUserRecordErr
	}

	m.Set(collectionmodels.FieldNameUsername, username)

	return nil
}

// Email returns the "email" users record data value.
func (m *Record) Email() string {
	return m.GetString(collectionmodels.FieldNameEmail)
}

// SetEmail sets the "email" users record data value.
//
...
func (m *Record) SetEmail(email string) error {
	...
	m.Set(collectionmodels.FieldNameEmail, email)

	return nil
}

// EmailVisibility returns the "emailVisibility" users record data value.
func (m *Record) EmailVisibility() bool {
	return m.GetBool(collectionmodels.FieldNameEmailVisibility)
}

// SetEmailVisibility sets the "emailVisibility" users record data value.
//
// Returns an error if the record is not from a users collection.
func (m *Record) SetEmailVisibility(visible bool) error {
	...
	m.Set(collectionmodels.FieldNameEmailVisibility, visible)

	return nil
}

// Verified returns the "verified" users record data value.
func (m *Record) Verified() bool {
	return m.GetBool(collectionmodels.FieldNameVerified)
}

// SetVerified sets the "verified" users record data value.
//
// Returns an error if the record is not from a users collection.
func (m *Record) SetVerified(verified bool) error {
	...
	m.Set(collectionmodels.FieldNameVerified, verified)

	return nil
}

// TokenKey returns the "tokenKey" users record data value.
func (m *Record) TokenKey() string {
	return m.GetString(collectionmodels.FieldNameTokenKey)
}

// SetTokenKey sets the "tokenKey" users record data value.
//
// Returns an error if the record is not from a users collection.
func (m *Record) SetTokenKey(key string) error {
	...
	m.Set(collectionmodels.FieldNameTokenKey, key)

	return nil
}

// RefreshTokenKey generates and sets new random users record "tokenKey".
//
// Returns an error if the record is not from a users collection.
...
// LastResetSentAt returns the "lastResentSentAt" users record data value.
func (m *Record) LastResetSentAt() types.DateTime {
	return m.GetDateTime(collectionmodels.FieldNameLastResetSentAt)
}

// SetLastResetSentAt sets the "lastResentSentAt" users record data value.
//
// Returns an error if the record is not from a users collection.
func (m *Record) SetLastResetSentAt(dateTime types.DateTime) error {
	...
	m.Set(collectionmodels.FieldNameLastResetSentAt, dateTime)

	return nil
}

// LastVerificationSentAt returns the "lastVerificationSentAt" users record data value.
func (m *Record) LastVerificationSentAt() types.DateTime {
	return m.GetDateTime(collectionmodels.FieldNameLastVerificationSentAt)
}

// SetLastVerificationSentAt sets a "lastVerificationSentAt" users record data value.
//
// Returns an error if the record is not from a users collection.
func (m *Record) SetLastVerificationSentAt(dateTime types.DateTime) error {
	...
	m.Set(collectionmodels.FieldNameLastVerificationSentAt, dateTime)

	return nil
}

// LastLoginAlertSentAt returns the "lastLoginAlertSentAt" users record data value.
func (m *Record) LastLoginAlertSentAt() types.DateTime {
	return m.GetDateTime(collectionmodels.FieldNameLastLoginAlertSentAt)
}

// SetLastLoginAlertSentAt sets a "lastLoginAlertSentAt" users record data value.
//
// Returns an error if the record is not from a users collection.
func (m *Record) SetLastLoginAlertSentAt(dateTime types.DateTime) error {
	...
	m.Set(collectionmodels.FieldNameLastLoginAlertSentAt, dateTime)

	return nil
}

// PasswordHash returns the "passwordHash" users record data value.
func (m *Record) PasswordHash() string {
	return m.GetString(collectionmodels.FieldNamePasswordHash)
}

// ValidatePassword validates a plain password against the users record password.
//
// Returns false if the password is incorrect or record is not from a users collection.
...
// SetPassword sets cryptographically secure string to the users record "password" field.
// This method also resets the "lastResetSentAt" and the "tokenKey" fields.
//
// Returns an error if the record is not from a users collection or
...
func (m *Record) SetPassword(password string) error {
	...
	m.Set(collectionmodels.FieldNamePasswordHash, string(hashedPassword))
	m.Set(collectionmodels.FieldNameLastResetSentAt, types.DateTime{})
