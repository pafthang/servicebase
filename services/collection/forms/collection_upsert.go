...
import (
	...
	"github.com/pafthang/servicebase/tools/dbutils"
...
var idRegex = baseforms.IdRegex

type InterceptorNextFunc[T any] = baseforms.InterceptorNextFunc[T]
type InterceptorFunc[T any] = baseforms.InterceptorFunc[T]
...
func runInterceptors[T any](
	...
	return baseforms.RunInterceptors(data, next, interceptors...)
}

type Upsert = CollectionUpsert
type UpsertInterceptorFunc = InterceptorFunc[*collectionmodels.Collection]

func NewUpsert(app core.App, collection *collectionmodels.Collection) *Upsert {
	return NewCollectionUpsert(app, collection)
}

// CollectionUpsert is a [collectionmodels.Collection] upsert (create/update) form.
type CollectionUpsert struct {
	...
	Schema     collectionmodels.Schema `form:"schema" json:"schema"`
...
// config created from the provided [core.App] and [collectionmodels.Collection] instances
// (for create you could pass a pointer to an empty Collection - `&collectionmodels.Collection{}`).
//
// If you want to submit the form as part of a transaction,
// you can change the default Dao via [SetDao()].
func NewCollectionUpsert(app core.App, collection *collectionmodels.Collection) *CollectionUpsert {
	...
	if form.Type == "" {
		form.Type = collectionmodels.CollectionTypeBase
	}

	clone, _ := form.collection.Schema.Clone()
	if clone != nil && form.Type != collectionmodels.CollectionTypeView {
		form.Schema = *clone
	} else {
		form.Schema = collectionmodels.Schema{}
	}
...
func (form *CollectionUpsert) Validate() error {
	isAuth := form.Type == collectionmodels.CollectionTypeUsers
	isView := form.Type == collectionmodels.CollectionTypeView

	// generate schema from the query (overwriting any explicit user defined schema)
	if isView {
		options := collectionmodels.CollectionViewOptions{}
	...
	return validation.ValidateStruct(form,
		validation.Field(
			...
			validation.When(
				...
				validation.Length(collectionmodels.DefaultIdLength, collectionmodels.DefaultIdLength),
				validation.Match(baseforms.IdRegex),
		...
		validation.Field(
			...
			validation.In(
				...
				collectionmodels.CollectionTypeView,
			),
...
func (form *CollectionUpsert) checkUniqueName(value any) error {
	...
	if form.dao.FindById(&collectionmodels.Collection{}, v) == nil {
...
func (form *CollectionUpsert) ensureNoFieldsTypeChange(value any) error {
	v, _ := value.(collectionmodels.Schema)
...
func (form *CollectionUpsert) checkRelationFields(value any) error {
	...
	for i, field := range v.Fields() {
		if field.Type != collectionmodels.FieldTypeRelation {
			continue
		}
		...
		oldField := form.collection.Schema.GetFieldById(field.Id)
		if oldField != nil {
			oldOptions, _ := oldField.Options.(*collectionmodels.RelationOptions)
		...
		if form.Type != collectionmodels.CollectionTypeView && relCollection.IsView() {
			return validation.Errors{fmt.Sprint(i): validation.Errors{
				"options": validation.Errors{
					"collectionId": validation.NewError(
...
func (form *CollectionUpsert) ensureNoAuthFieldName(value any) error {
	...
	if form.Type != collectionmodels.CollectionTypeUsers {
		return nil // not a users collection
	}

	authFieldNames := collectionmodels.UserFieldNames()
...
func (form *CollectionUpsert) checkMinSchemaFields(value any) error {
	...
	switch form.Type {
	case collectionmodels.CollectionTypeUsers, collectionmodels.CollectionTypeView:
...
func (form *CollectionUpsert) checkIndexes(value any) error {
	...
	if form.Type == collectionmodels.CollectionTypeView && len(v) > 0 {
...
func (form *CollectionUpsert) checkOptions(value any) error {
	...
	switch form.Type {
	case collectionmodels.CollectionTypeUsers:
		options := collectionmodels.CollectionUserOptions{}
	...
	case collectionmodels.CollectionTypeView:
		options := collectionmodels.CollectionViewOptions{}
		if err := decodeOptions(v, &options); err != nil {
			return err
		}
...
func (form *CollectionUpsert) Submit(interceptors ...baseforms.InterceptorFunc[*collectionmodels.Collection]) error {
	...
	return baseforms.RunInterceptors(form.collection, func(collection *collectionmodels.Collection) error {
