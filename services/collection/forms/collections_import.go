...
type CollectionsImport struct {
	...
	Collections   []*collectionmodels.Collection `form:"collections" json:"collections"`
	DeleteMissing bool                           `form:"deleteMissing" json:"deleteMissing"`
}

type Import = CollectionsImport
type ImportInterceptorFunc = baseforms.InterceptorFunc[[]*collectionmodels.Collection]
...
func NewImport(app core.App) *Import {
	return NewCollectionsImport(app)
}
...
func (form *CollectionsImport) Submit(interceptors ...baseforms.InterceptorFunc[[]*collectionmodels.Collection]) error {
	if err := form.Validate(); err != nil {
		return err
	}

	return baseforms.RunInterceptors(form.Collections, func(collections []*collectionmodels.Collection) error {
...
func (form *CollectionsImport) afterSync(txDao *daos.Dao, mappedNew, mappedOld map[string]*collectionmodels.Collection) error {
	// refresh the actual persisted collections list
	refreshedCollections := []*collectionmodels.Collection{}
