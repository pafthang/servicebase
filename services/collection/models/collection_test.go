...
func TestCollectionIsBase(t *testing.T) {
	...
	scenarios := []struct {
		...
		{models.Collection{Type: models.CollectionTypeUsers}, false},
	}
...
func TestCollectionIsUsers(t *testing.T) {
	t.Parallel()
	...
	scenarios := []struct {
		...
		{models.Collection{Type: models.CollectionTypeUsers}, true},
	}

	for i, s := range scenarios {
		result := s.collection.IsUsers()
		if result != s.expected {
			t.Errorf("(%d) Expected %v, got %v", i, s.expected, result)
		}
	}
...
func TestCollectionMarshalJSON(t *testing.T) {
	...
	scenarios := []struct {
		...
		{
			"users type + non empty options",
			models.Collection{BaseModel: models.BaseModel{Id: "test"}, Type: models.CollectionTypeUsers, Options: types.JsonMap{"test": 123, "allowOAuth2Auth": true, "minPasswordLength": 4, "onlyVerified": true}},
...
func TestCollectionBaseOptions(t *testing.T) {
	...
	scenarios := []struct {
		...
		{
			"different type",
			models.Collection{Type: models.CollectionTypeUsers, Options: types.JsonMap{"test": 123, "minPasswordLength": 4}},
			"{}",
		},
		{
			"base type",
...
func TestCollectionUserOptions(t *testing.T) {
	t.Parallel()
	...
	scenarios := []struct {
		...
		{
			"users type",
			models.Collection{Type: models.CollectionTypeUsers, Options: options},
			expectedSerialization,
		},
	}
	...
	for _, s := range scenarios {
		t.Run(s.name, func(t *testing.T) {
			result := s.collection.UserOptions()
...
func TestSetOptions(t *testing.T) {
	...
	scenarios := []struct {
		...
		{
			"auth type",
			models.Collection{Type: models.CollectionTypeUsers, Options: types.JsonMap{"test": 123}},
			map[string]any{"test": 456, "minPasswordLength": 4},
			`{"allowEmailAuth":false,"allowOAuth2Auth":false,"allowUsernameAuth":false,"exceptEmailDomains":null,"manageRule":null,"minPasswordLength":4,"onlyEmailDomains":null,"onlyVerified":false,"requireEmail":false}`,
		},
	}
...
func TestCollectionUserOptionsValidate(t *testing.T) {
	t.Parallel()

	scenarios := []struct {
		name           string
		options        models.CollectionUserOptions
		...
		},
		{
			...
			models.CollectionUserOptions{ManageRule: types.Pointer("")},
			[]string{"manageRule"},
		},
		{
			"minPasswordLength < 5",
			models.CollectionUserOptions{MinPasswordLength: 3},
			[]string{"minPasswordLength"},
		},
		{
			"minPasswordLength > 72",
			models.CollectionUserOptions{MinPasswordLength: 73},
			[]string{"minPasswordLength"},
		},
		{
