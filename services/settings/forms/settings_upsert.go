...
// Submit validates the form and upserts the loaded models.
//
// On success the app settings will be refreshed with the form ones.
//
// You can optionally provide a list of InterceptorFunc to further
// modify the form behavior before persisting it.
func (form *SettingsUpsert) Submit(interceptors ...forms.InterceptorFunc[*settingsmodels.Settings]) error {
	if err := form.Validate(); err != nil {
		return err
	}

	return forms.RunInterceptors(form.Settings, func(s *settingsmodels.Settings) error {
		form.Settings = s
		...
		form.app.LogsDao().NonconcurrentDB().Delete((&logmodels.Log{}).TableName(), expr).Execute()
