...
func TestRecordEmailChangeRequestValidateAndSubmit(t *testing.T) {
	...
	user, err := testApp.Dao().FindUserRecordByEmail("users", "test@example.com")
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, s := range scenarios {
		...
		form := userforms.NewRecordEmailChangeRequest(testApp, user)
...
func TestRecordEmailChangeRequestInterceptors(t *testing.T) {
	...
	form := userforms.NewRecordEmailChangeRequest(testApp, authRecord)
