...
func TestRecordPasswordLoginValidateAndSubmit(t *testing.T) {
	...
	scenarios := []struct {
		...
		{
			"existing username + valid password but in restricted username users collection",
			"clients",
			"clients57772",
			"1234567890",
			true,
		...
		{
			"existing username + valid password but in restricted username and email users collection",
			"nologin",
			"test_username",
			"1234567890",
			true,
	...
	for _, s := range scenarios {
		...
		if err != nil {
			t.Errorf("[%s] Failed to fetch users collection: %v", s.testName, err)
		}

		form := userforms.NewRecordPasswordLogin(testApp, authCollection)
...
func TestRecordPasswordLoginInterceptors(t *testing.T) {
	...
	var interceptorRecord *recordmodels.Record
	...
	interceptor2 := func(next baseforms.InterceptorNextFunc[*recordmodels.Record]) baseforms.InterceptorNextFunc[*models.Record] {
		return func(record *recordmodels.Record) error {
