...
func TestRecordPasswordResetRequestSubmit(t *testing.T) {
	...
	for i, s := range scenarios {
		...
		form := userforms.NewRecordPasswordResetRequest(testApp, authCollection)
		...
		interceptorCalls := 0
		interceptor := func(next baseforms.InterceptorNextFunc[*recordmodels.Record]) baseforms.InterceptorNextFunc[*models.Record] {
		...
		user, err := testApp.Dao().FindUserRecordByEmail(authCollection.Id, form.Email)
		if err != nil {
...
func TestRecordPasswordResetRequestInterceptors(t *testing.T) {
	...
	interceptor1 := func(next baseforms.InterceptorNextFunc[*models.Record]) baseforms.InterceptorNextFunc[*models.Record] {
	...
	interceptor2 := func(next baseforms.InterceptorNextFunc[*models.Record]) baseforms.InterceptorNextFunc[*models.Record] {
		return func(record *models.Record) error {
