...
func TestUserOauth2LoginValidate(t *testing.T) {
	...
	app.Settings().GithubAuth.Enabled = true
	...
	scenarios := []struct {
		...
		},
		{
			"disabled provider",
			"users",
			`{"provider":"vk","code":"123","codeVerifier":"123","redirectUrl":"https://example.com"}`,
			[]string{"provider"},
		},
		{
			"enabled provider",
		...
		},
		{
			...
			"users",
			`{"provider":"github","code":"123","codeVerifier":"123","redirectUrl":"something"}`,
			[]string{},
		},
	}
	...
	for _, s := range scenarios {
		...
		if authCollection == nil {
			t.Errorf("[%s] Failed to fetch users collection", s.testName)
		}

		form := userforms.NewRecordOAuth2Login(app, authCollection, nil)
