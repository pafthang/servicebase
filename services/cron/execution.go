...
func NewExecutor() *Executor {
	return NewExecutorWithApp(nil)
}

func NewExecutorWithApp(app core.App) *Executor {
	return &Executor{
		client: &http.Client{
...
func (e *Executor) Execute(cronRecord *cronmodels.Cron) error {
	startTime := time.Now()
	...
	if err := e.logExecution(cronRecord, result); err != nil {
		e.logger.Error("Failed to log execution", "error", err)
	}
	...
	if err := saveCron(e.app, cronRecord); err != nil {
		e.logger.Error("Failed to update cron last_run", "error", err)
	}
...
func (e *Executor) logExecution(cronRecord *cronmodels.Cron, result *ExecutionResult) error {
	execution := &cronmodels.CronExecution{
		Cron:          cronRecord.Id,
	...
	if err := saveCronExecution(e.app, execution); err != nil {
...
func (e *Executor) scheduleRetry(cronRecord *cronmodels.Cron, result *ExecutionResult) {
	...
	cronRecord, err := findCronByID(e.app, cronRecord.Id)
	if err != nil {
		e.logger.Error("Failed to reload cron for retry", "error", err)
		return
	}
	...
	if err := e.Execute(cronRecord); err != nil {
		e.logger.Error("Retry execution failed", "cronId", cronRecord.Id, "error", err)
	}
}

func (e *Executor) sendNotification(cronRecord *cronmodels.Cron, result *ExecutionResult) error {
