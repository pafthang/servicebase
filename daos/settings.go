...
func (dao *Dao) FindSettings(optEncryptionKey ...string) (*settingsmodels.Settings, error) {
	param, err := dao.FindParamByKey(settingsmodels.ParamAppSettings)
	if err != nil {
		return nil, err
	}

	result := settingsmodels.NewSettings()
...
func (dao *Dao) SaveSettings(newSettings *settingsmodels.Settings, optEncryptionKey ...string) error {
	return dao.SaveParam(settingsmodels.ParamAppSettings, newSettings, optEncryptionKey...)
}
