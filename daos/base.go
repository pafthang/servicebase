...
import (
	...
	"github.com/pafthang/servicebase/services/base/models"
...
func (dao *Dao) copyConfigTo(target *Dao) {
	target.MaxLockRetries = dao.MaxLockRetries
	target.ModelQueryTimeout = dao.ModelQueryTimeout
	target.BeforeCreateFunc = dao.BeforeCreateFunc
	target.AfterCreateFunc = dao.AfterCreateFunc
	target.BeforeUpdateFunc = dao.BeforeUpdateFunc
	target.AfterUpdateFunc = dao.AfterUpdateFunc
	target.BeforeDeleteFunc = dao.BeforeDeleteFunc
	target.AfterDeleteFunc = dao.AfterDeleteFunc
}
	...
	return dao.lockRetry(func(retryDao *Dao) error {
		action := func() error {
			...
			if retryDao.AfterDeleteFunc != nil {
				return retryDao.AfterDeleteFunc(retryDao, m)
			}
...
func (dao *Dao) lockRetry(op func(retryDao *Dao) error) error {
	...
	return baseLockRetry(func(attempt int) error {
		if attempt == 2 {
			...
			retryDao.MaxLockRetries = dao.MaxLockRetries
			retryDao.ModelQueryTimeout = dao.ModelQueryTimeout
