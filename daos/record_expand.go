...
type ExpandFetchFunc func(relCollection *collectionmodels.Collection, relIds []string) ([]*recordmodels.Record, error)
...
func (dao *Dao) ExpandRecord(record *recordmodels.Record, expands []string, optFetchFunc ExpandFetchFunc) map[string]error {
	return dao.ExpandRecords([]*recordmodels.Record{record}, expands, optFetchFunc)
}
...
//
// Returns a map with the failed expand parameters and their errors.
func (dao *Dao) ExpandRecords(records []*recordmodels.Record, expands []string, optFetchFunc ExpandFetchFunc) map[string]error {
...
func (dao *Dao) expandRecords(records []*recordmodels.Record, expandPath string, fetchFunc ExpandFetchFunc, recursionLevel int) error {
	if fetchFunc == nil {
		// load a default fetchFunc
		fetchFunc = func(relCollection *collectionmodels.Collection, relIds []string) ([]*recordmodels.Record, error) {
	...
	var relField *collectionmodels.SchemaField
	var relFieldOptions *collectionmodels.RelationOptions
	var relCollection *collectionmodels.Collection
	...
	if len(matches) == 3 {
		...
		if indirectRelField == nil || indirectRelField.Type != collectionmodels.FieldTypeRelation {
		...
		indirectRelFieldOptions, _ := indirectRelField.Options.(*collectionmodels.RelationOptions)
		...
		relFieldOptions = &collectionmodels.RelationOptions{
			MaxSelect:    nil,
		...
		relField = &collectionmodels.SchemaField{
		...
		if relField == nil || relField.Type != collectionmodels.FieldTypeRelation {
			return fmt.Errorf("Couldn't find relation field %q in collection %q.", parts[0], mainCollection.Name)
		}
		relField.InitOptions()
		relFieldOptions, _ = relField.Options.(*collectionmodels.RelationOptions)
	...
	indexedRels := make(map[string]*recordmodels.Record, len(rels))
	...
	for _, model := range records {
		...
		validRels := make([]*recordmodels.Record, 0, len(relIds))
		...
		var oldExpandedRels []*recordmodels.Record
		switch v := expandData[relField.Name].(type) {
		case nil:
			// no old expands
		case *recordmodels.Record:
			oldExpandedRels = []*recordmodels.Record{v}
		case []*recordmodels.Record:
			oldExpandedRels = v
		}
...
func isRelFieldUnique(collection *collectionmodels.Collection, fieldName string) bool {
