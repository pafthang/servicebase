...
import (
	...
	"github.com/pafthang/servicebase/tools/tokenizer"
...
func (dao *Dao) CreateViewSchema(selectQuery string) (collectionmodels.Schema, error) {
	result := collectionmodels.NewSchema()
	...
	txErr := dao.RunInTransaction(func(txDao *Dao) error {
		...
		for _, row := range info {
			if row.Name == collectionmodels.FieldNameId {
				hasId = true
			}

			if list.ExistInSlice(row.Name, collectionmodels.BaseModelFieldNames()) {
				continue // skip base model fields since they are not part of the schema
			}

			var field *collectionmodels.SchemaField
...
func (dao *Dao) FindRecordByViewFile(
	...
	if opt, ok := qf.original.Options.(collectionmodels.MultiValuer); !ok || !opt.IsMultiple() {
...
type queryField struct {
	...
	// It could be nil if the found query field is not from a collection models.
	collection *collectionmodels.Collection

	// original is the original found collection field.
	// It could be nil if the found query field is not from a collection models.
	original *collectionmodels.SchemaField
}

func defaultViewField(name string) *collectionmodels.SchemaField {
	return &collectionmodels.SchemaField{
		Name: name,
		Type: collectionmodels.FieldTypeJson,
		Options: &collectionmodels.JsonOptions{
...
func (dao *Dao) parseQueryToFields(selectQuery string) (map[string]*queryField, error) {
	...
	for _, col := range p.columns {
		...
		if fieldName == collectionmodels.FieldNameId {
		...
		} else if fieldName == collectionmodels.FieldNameCreated || fieldName == collectionmodels.FieldNameUpdated {
			result[col.alias] = &queryField{
				field: &collectionmodels.SchemaField{
					Name: col.alias,
					Type: collectionmodels.FieldTypeDate,
				},
				collection: collection,
			}
		} else if fieldName == collectionmodels.FieldNameUsername && collection.IsUsers() {
			result[col.alias] = &queryField{
				field: &collectionmodels.SchemaField{
					Name: col.alias,
					Type: collectionmodels.FieldTypeText,
		...
		} else if fieldName == collectionmodels.FieldNameEmail && collection.IsUsers() {
			result[col.alias] = &queryField{
				field: &collectionmodels.SchemaField{
					Name: col.alias,
					Type: collectionmodels.FieldTypeEmail,
				},
				collection: collection,
			}
		} else if (fieldName == collectionmodels.FieldNameVerified || fieldName == collectionmodels.FieldNameEmailVisibility) && collection.IsUsers() {
			result[col.alias] = &queryField{
				field: &collectionmodels.SchemaField{
					Name: col.alias,
					Type: collectionmodels.FieldTypeBool,
...
func (dao *Dao) findCollectionsByIdentifiers(tables []identifier) (map[string]*collectionmodels.Collection, error) {
	...
	result := make(map[string]*collectionmodels.Collection, len(names))
	collections := make([]*collectionmodels.Collection, 0, len(names))
