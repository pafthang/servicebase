...
func (dao *Dao) CollectionQuery() *dbx.SelectQuery {
	return dao.ModelQuery(&collectionmodels.Collection{})
}

// FindCollectionsByType finds all collections by the given type.
func (dao *Dao) FindCollectionsByType(collectionType string) ([]*collectionmodels.Collection, error) {
...
func (dao *Dao) FindCollectionByNameOrId(nameOrId string) (*collectionmodels.Collection, error) {
...
func (dao *Dao) FindCollectionReferences(collection *collectionmodels.Collection, excludeIds ...string) (map[*collectionmodels.Collection][]*collectionmodels.SchemaField, error) {
	collections := []*collectionmodels.Collection{}
	...
	result := map[*collectionmodels.Collection][]*collectionmodels.SchemaField{}

	for _, c := range collections {
		for _, f := range c.Schema.Fields() {
			if f.Type != collectionmodels.FieldTypeRelation {
				continue
			}
			f.InitOptions()
			options, _ := f.Options.(*collectionmodels.RelationOptions)
...
func (dao *Dao) DeleteCollection(collection *collectionmodels.Collection) error {
...
func (dao *Dao) SaveCollection(collection *collectionmodels.Collection) error {
	var oldCollection *collectionmodels.Collection
	...
	txErr := dao.RunInTransaction(func(txDao *Dao) error {
		...
		if collection.Type == "" {
			collection.Type = collectionmodels.CollectionTypeBase
		}
...
func (dao *Dao) ImportCollections(
	importedCollections []*collectionmodels.Collection,
	deleteMissing bool,
	afterSync func(txDao *Dao, mappedImported, mappedExisting map[string]*collectionmodels.Collection) error,
) error {
	...
	return dao.RunInTransaction(func(txDao *Dao) error {
		existingCollections := []*collectionmodels.Collection{}
		if err := txDao.CollectionQuery().OrderBy("updated ASC").All(&existingCollections); err != nil {
			return err
		}
		mappedExisting := make(map[string]*collectionmodels.Collection, len(existingCollections))
		for _, existing := range existingCollections {
			mappedExisting[existing.GetId()] = existing
		}

		mappedImported := make(map[string]*collectionmodels.Collection, len(importedCollections))
		for _, imported := range importedCollections {
			...
			if imported.Type == "" {
				imported.Type = collectionmodels.CollectionTypeBase
			}
		...
		if deleteMissing {
			for _, existing := range existingCollections {
				...
				if existing.System && isInternalTeamCollection(existing) {
					continue
				}
...
func isInternalTeamCollection(collection *collectionmodels.Collection) bool {
	if collection == nil {
		return false
	}

	return collection.Name == (&teammodels.Team{}).TableName() ||
		collection.Name == (&teammodels.TeamMember{}).TableName()
}
...
func (dao *Dao) saveViewCollection(newCollection, oldCollection *collectionmodels.Collection) error {
...
func (dao *Dao) normalizeViewQueryId(query string) (string, error) {
	...
	idField := parsed[collectionmodels.FieldNameId]
	if idField != nil && idField.field != nil &&
		idField.field.Type != collectionmodels.FieldTypeJson &&
		idField.field.Type != collectionmodels.FieldTypeNumber &&
		idField.field.Type != collectionmodels.FieldTypeBool {
		needWrapping = false
	}
	...
	for _, col := range rawParsed.columns {
		if col.alias == collectionmodels.FieldNameId {
...
func (dao *Dao) resaveViewsWithChangedSchema(excludeIds ...string) error {
	collections, err := dao.FindCollectionsByType(collectionmodels.CollectionTypeView)
	if err != nil {
		return err
	}
