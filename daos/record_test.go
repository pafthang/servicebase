...
func TestCanAccessRecord(t *testing.T) {
	...
	authRecord, err := app.Dao().FindUserRecordByEmail("users", "test@example.com")
	if err != nil {
		t.Fatal(err)
	}
	...
	scenarios := []struct {
		...
		{
			"as users record with empty rule",
			record,
			&models.RequestInfo{
				AuthRecord: authRecord,
			},
		...
		{
			"as users record with invalid rule",
			record,
			&models.RequestInfo{
				AuthRecord: authRecord,
			},
		...
		{
			"as users record with mismatched rule",
			record,
			&models.RequestInfo{
				AuthRecord: authRecord,
				Data:       map[string]any{"test": 1},
		...
		{
			"as users record with matched rule",
			record,
			&models.RequestInfo{
				AuthRecord: authRecord,
...
func TestFindUserRecordByToken(t *testing.T) {
	t.Parallel()
	...
	for i, scenario := range scenarios {
		record, err := app.Dao().FindUserRecordByToken(scenario.token, scenario.baseKey)
...
func TestFindUserRecordByEmail(t *testing.T) {
	t.Parallel()
	...
	for i, scenario := range scenarios {
		record, err := app.Dao().FindUserRecordByEmail(scenario.collectionIdOrName, scenario.email)
...
func TestFindUserRecordByUsername(t *testing.T) {
	t.Parallel()
	...
	for i, scenario := range scenarios {
		record, err := app.Dao().FindUserRecordByUsername(scenario.collectionIdOrName, scenario.username)
...
func TestSuggestUniqueUserRecordUsername(t *testing.T) {
	t.Parallel()
	...
	scenarios := []struct {
		...
		// not an users collection
		{"demo2", "test2_username", `^test2_username\d{12}$`},
		// users collection with unique base username
		{"users", "new_username", `^new_username$`},
		{"users", "NEW_USERNAME", `^NEW_USERNAME$`},
		// users collection with existing username
		{"users", "test2_username", `^test2_username\d{3}$`},
		{"users", "TEST2_USERNAME", `^TEST2_USERNAME\d{3}$`},
	}

	for i, scenario := range scenarios {
		username := app.Dao().SuggestUniqueUserRecordUsername(
			scenario.collectionIdOrName,
...
func createMockBatchProcessingData(dao *daos.Dao) error {
	...
	c1 := &collectionmodels.Collection{}
	c1.Id = "c1"
	c1.Name = c1.Id
	c1.Schema = collectionmodels.NewSchema(
	...
	c2 := &collectionmodels.Collection{}
	c2.Id = "c2"
	c2.Name = c2.Id
	c2.Schema = collectionmodels.NewSchema(
		&collectionmodels.SchemaField{
			Name: "rel",
			Type: collectionmodels.FieldTypeRelation,
			Options: &collectionmodels.RelationOptions{
	...
	c3 := &collectionmodels.Collection{}
	c3.Id = "c3"
	c3.Name = c3.Id
	c3.Schema = collectionmodels.NewSchema(
		&collectionmodels.SchemaField{
			Name: "rel",
			Type: collectionmodels.FieldTypeRelation,
			Options: &collectionmodels.RelationOptions{
