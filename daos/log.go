...
func (dao *Dao) LogQuery() *dbx.SelectQuery {
	return dao.ModelQuery(&logmodels.Log{})
}

// FindLogById finds a single Log entry by its id.
func (dao *Dao) FindLogById(id string) (*logmodels.Log, error) {
	model := &logmodels.Log{}
...
func (dao *Dao) DeleteOldLogs(createdBefore time.Time) error {
	...
	_, err := dao.NonconcurrentDB().Delete((&logmodels.Log{}).TableName(), expr).Execute()

	return err
}

// SaveLog upserts the provided Log model.
func (dao *Dao) SaveLog(log *logmodels.Log) error {
	return dao.Save(log)
}
