...
func (dao *Dao) SyncRecordTableSchema(newCollection *collectionmodels.Collection, oldCollection *collectionmodels.Collection) error {
	return dao.RunInTransaction(func(txDao *Dao) error {
		// create
		// -----------------------------------------------------------
		if oldCollection == nil {
			cols := map[string]string{
				collectionmodels.FieldNameId:      "TEXT PRIMARY KEY DEFAULT ('r'||lower(hex(randomblob(7)))) NOT NULL",
				collectionmodels.FieldNameCreated: "TEXT DEFAULT (strftime('%Y-%m-%d %H:%M:%fZ')) NOT NULL",
				collectionmodels.FieldNameUpdated: "TEXT DEFAULT (strftime('%Y-%m-%d %H:%M:%fZ')) NOT NULL",
			}

			if newCollection.IsUsers() {
				cols[collectionmodels.FieldNameUsername] = "TEXT NOT NULL"
				cols[collectionmodels.FieldNameEmail] = "TEXT DEFAULT '' NOT NULL"
				cols[collectionmodels.FieldNameEmailVisibility] = "BOOLEAN DEFAULT FALSE NOT NULL"
				cols[collectionmodels.FieldNameVerified] = "BOOLEAN DEFAULT FALSE NOT NULL"
				cols[collectionmodels.FieldNameTokenKey] = "TEXT NOT NULL"
				cols[collectionmodels.FieldNamePasswordHash] = "TEXT NOT NULL"
				cols[collectionmodels.FieldNameLastResetSentAt] = "TEXT DEFAULT '' NOT NULL"
				cols[collectionmodels.FieldNameLastVerificationSentAt] = "TEXT DEFAULT '' NOT NULL"
				cols[collectionmodels.FieldNameLastLoginAlertSentAt] = "TEXT DEFAULT '' NOT NULL"
			}
...
func (dao *Dao) normalizeSingleVsMultipleFieldChanges(newCollection, oldCollection *collectionmodels.Collection) error {
	...
	return dao.RunInTransaction(func(txDao *Dao) error {
		...
		for _, newField := range newCollection.Schema.Fields() {
			...
			if oldField := oldCollection.Schema.GetFieldById(newField.Id); oldField != nil {
				if opt, ok := oldField.Options.(collectionmodels.MultiValuer); ok {
					isOldMultiple = opt.IsMultiple()
				}
			}

			var isNewMultiple bool
			if opt, ok := newField.Options.(collectionmodels.MultiValuer); ok {
...
func (dao *Dao) dropCollectionIndex(collection *collectionmodels.Collection) error {
...
func (dao *Dao) createCollectionIndexes(collection *collectionmodels.Collection) error {
	if collection.IsView() {
		return nil // views don't have indexes
	}
