...
import (
	...
	settingmodels "github.com/pafthang/servicebase/services/settings/models"
)

// ParamQuery returns a new Param select query.
func (dao *Dao) ParamQuery() *dbx.SelectQuery {
	return dao.ModelQuery(&settingmodels.Param{})
}

// FindParamByKey finds the first Param model with the provided key.
func (dao *Dao) FindParamByKey(key string) (*settingmodels.Param, error) {
	param := &settingmodels.Param{}
...
func (dao *Dao) SaveParam(key string, value any, optEncryptionKey ...string) error {
	...
	if param == nil {
		param = &settingmodels.Param{Key: key}
	}
...
func (dao *Dao) DeleteParam(param *settingmodels.Param) error {
