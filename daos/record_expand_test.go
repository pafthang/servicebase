...
func TestExpandRecords(t *testing.T) {
	...
	scenarios := []struct {
	...
	}{
		{
			...
			func(c *collectionmodels.Collection, ids []string) ([]*models.Record, error) {
	...
	for _, s := range scenarios {
		...
		totalExpandProps := strings.Count(encodedStr, collectionmodels.FieldNameExpand)
...
func TestIndirectExpandSingeVsArrayResult(t *testing.T) {
	...
	{
		errs := app.Dao().ExpandRecord(record, []string{"demo4_via_rel_one_cascade"}, func(c *collectionmodels.Collection, ids []string) ([]*models.Record, error) {
			return app.Dao().FindRecordsByIds(c.Id, ids, nil)
		})
		if len(errs) > 0 {
			t.Fatal(errs)
