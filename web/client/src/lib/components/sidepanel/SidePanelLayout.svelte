<script lang="ts">
  ...
  import { invoke, listen, windowHide, windowStartDragging } from "$lib/native/runtime";
  import {
    emitSidePanelReady,
    onSessionSwitch,
    onChatSync,
  ...
  onMount(async () => {
    ...
    if (isTauri()) {
      try {
        const unsub = await listen<{ token: string }>("token-updated", (payload) => {
          connectionStore.updateToken(payload.token);
        });
