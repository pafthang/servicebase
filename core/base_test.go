...
import (
	...
	"github.com/pafthang/servicebase/tools/logger"
...
func TestBaseAppGetters(t *testing.T) {
	...
	if app.dao.ConcurrentDB() != app.Dao().DB() {
		t.Fatalf("Expected app.DB %v, got %v", app.Dao().DB(), app.dao.ConcurrentDB())
	}
	...
	if app.logsDao.ConcurrentDB() != app.LogsDao().DB() {
		t.Fatalf("Expected app.LogsDB %v, got %v", app.LogsDao().DB(), app.logsDao.ConcurrentDB())
	}
	...
	for _, s := range scenarios {
		t.Run(s.name, func(t *testing.T) {
			...
			for level, enabled := range s.expectations {
				if v := handler.Enabled(context.TODO(), slog.Level(level)); v != enabled {
...
func initTestBaseApp() (app *BaseApp, cleanup func(), err error) {
	...
	initErr := func() error {
		...
		if !ok {
			return fmt.Errorf("logs db is not *dbx.DB")
		}

		dataDB, ok := app.Dao().DB().(*dbx.DB)
		if !ok {
			return fmt.Errorf("data db is not *dbx.DB")
		}

		logsRunner, err := migrate.NewRunner(logsDB, registry.LogsMigrations)
		if err != nil {
		...
		dataRunner, err := migrate.NewRunner(dataDB, registry.AppMigrations)
		if err != nil {
			return fmt.Errorf("dataRunner error: %w", err)
		}
