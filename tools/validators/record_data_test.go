...
func TestRecordDataValidatorValidateText(t *testing.T) {
	...
	max := 10
	pattern := `^\w+$`
	collection.Schema = models.NewSchema(
		&models.SchemaField{
			Name: "field1",
	...
	dummy := recordmodels.NewRecord(collection)
	...
	checkValidatorErrors(t, app.Dao(), recordmodels.NewRecord(collection), scenarios)
}
...
func TestRecordDataValidatorValidateRelation(t *testing.T) {
	...
	collection.Schema = models.NewSchema(
		&models.SchemaField{
			...
			Options: &models.RelationOptions{
...
func checkValidatorErrors(t *testing.T, dao *daos.Dao, record *recordmodels.Record, scenarios []testDataFieldScenario) {
