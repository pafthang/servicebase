...
func (r *runner) run() (*search.ResolverResult, error) {
	...
	if r.activeProps[0] == "@request" {
		...
		if strings.HasPrefix(r.fieldName, "@request.data.") && len(r.activeProps) > 2 {
			...
			if dataField.Type == models.FieldTypeRelation && len(r.activeProps) > 3 {
				return r.processRequestInfoRelationField(dataField)
			}

			// check for data arrayble fields ":each" modifier
			if modifier == eachModifier && list.ExistInSlice(dataField.Type, models.ArraybleFieldTypes()) && len(r.activeProps) == 3 {
				return r.processRequestInfoEachModifier(dataField)
			}

			// check for data arrayble fields ":length" modifier
			if modifier == lengthModifier && list.ExistInSlice(dataField.Type, models.ArraybleFieldTypes()) && len(r.activeProps) == 3 {
...
func (r *runner) processRequestInfoLengthModifier(dataField *models.SchemaField) (*search.ResolverResult, error) {
...
func (r *runner) processRequestInfoEachModifier(dataField *models.SchemaField) (*search.ResolverResult, error) {
	options, ok := dataField.Options.(models.MultiValuer)
	if !ok {
...
func (r *runner) processRequestInfoRelationField(dataField *models.SchemaField) (*search.ResolverResult, error) {
	options, ok := dataField.Options.(*models.RelationOptions)
	if !ok {
...
func (r *runner) processActiveProps() (*search.ResolverResult, error) {
	...
	for i, prop := range r.activeProps {
		...
		if i == totalProps-1 {
			...
			// -------------------------------------------------------
			if modifier == lengthModifier && list.ExistInSlice(field.Type, models.ArraybleFieldTypes()) {
			...
			// -------------------------------------------------------
			if modifier == eachModifier && list.ExistInSlice(field.Type, models.ArraybleFieldTypes()) {
				jePair := r.activeTableAlias + "." + cleanFieldName
				...
				options, ok := field.Options.(models.MultiValuer)
				if !ok {
			...
			if field.Type == models.FieldTypeJson {
		...
		if field != nil && field.Type == models.FieldTypeJson {
		...
		if field == nil {
			...
			if backField.Type != models.FieldTypeRelation {
				return nil, fmt.Errorf("invalid back relation field %q", parts[2])
			}

			backField.InitOptions()
			backFieldOptions, ok := backField.Options.(*models.RelationOptions)
			if !ok {
		...
		if field.Type != models.FieldTypeRelation {
		...
		// ---
		field.InitOptions()
		options, ok := field.Options.(*models.RelationOptions)
		if !ok {
...
func resolvableSystemFieldNames(collection *models.Collection) []string {
	result := models.BaseModelFieldNames()

	if collection.IsUsers() {
		result = append(
			result,
			models.FieldNameUsername,
			models.FieldNameVerified,
			models.FieldNameEmailVisibility,
			models.FieldNameEmail,
		)
	}
