...
var plainRequestAuthFields = []string{
	"@request.auth." + collectionmodels.FieldNameId,
	"@request.auth." + collectionmodels.FieldNameCollectionId,
	"@request.auth." + collectionmodels.FieldNameCollectionName,
	"@request.auth." + collectionmodels.FieldNameUsername,
	"@request.auth." + collectionmodels.FieldNameEmail,
	"@request.auth." + collectionmodels.FieldNameEmailVisibility,
	"@request.auth." + collectionmodels.FieldNameVerified,
	"@request.auth." + collectionmodels.FieldNameCreated,
	"@request.auth." + collectionmodels.FieldNameUpdated,
}
...
type CollectionsFinder interface {
	FindCollectionByNameOrId(collectionNameOrId string) (*collectionmodels.Collection, error)
}
...
type RecordFieldResolver struct {
	...
	baseCollection    *collectionmodels.Collection
	requestInfo       *recordmodels.RequestInfo
	staticRequestInfo map[string]any
	allowedFields     []string
	loadedCollections []*collectionmodels.Collection
	joins             []*join
...
func NewRecordFieldResolver(
	...
	r := &RecordFieldResolver{
		...
		joins:             []*join{},
		loadedCollections: []*collectionmodels.Collection{baseCollection},
		allowedFields: []string{
			`^\w+[\w\.\:]*$`,
			`^\@request\.context$`,
			`^\@request\.method$`,
	...
	if r.requestInfo != nil {
		...
		if r.requestInfo.AuthRecord != nil {
			...
			authData[collectionmodels.FieldNameEmail] = r.requestInfo.AuthRecord.Email()
...
func (r *RecordFieldResolver) resolveStaticRequestField(path ...string) (*search.ResolverResult, error) {
	...
	case string:
		...
		if field != nil && field.Type == collectionmodels.FieldTypeNumber {
...
func (r *RecordFieldResolver) loadCollection(collectionNameOrId string) (*collectionmodels.Collection, error) {
