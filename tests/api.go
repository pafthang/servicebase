...
const TestAdminTeamAccessAuth = "test_admin_team_access"
...
func (scenario *ApiScenario) test(t *testing.T) {
	...
	services, err := appinit.NewServices(appinit.ServicesConfig{App: testApp})
	if err != nil {
		t.Fatal(err)
	}

	e, err := appinit.InitRouter(testApp, services)
	if err != nil {
		t.Fatal(err)
	}
	...
	prepareAdminTeamAccessAuthHeader(t, testApp, req)
...
func prepareAdminTeamAccessAuthHeader(t *testing.T, testApp *TestApp, req *http.Request) {
	token := req.Header.Get("Authorization")
	if token != TestAdminTeamAccessAuth {
		return
	}

	record, err := testApp.Dao().FindUserRecordByEmail("users", "test@example.com")
	if err != nil {
		t.Fatalf("failed to load admin replacement auth record: %v", err)
	}

	if err := teamservice.New(testApp).GrantAdminTeamAccess(record); err != nil {
		t.Fatalf("failed to grant admin team access to replacement auth record: %v", err)
	}

	recordToken, err := tokens.NewRecordAuthToken(testApp, record)
	if err != nil {
		t.Fatalf("failed to create replacement auth token: %v", err)
	}

	req.Header.Set("Authorization", recordToken)
